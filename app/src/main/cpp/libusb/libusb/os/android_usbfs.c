...
static int _get_usbfs_fd(struct libusb_device *device, mode_t mode, int silent) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "hola _get_usbfs_fd dpriv->fd %d", dpriv->fd);
...
static int submit_control_transfer(struct usbi_transfer *itransfer) {
    ...
    r = ioctl(dpriv->fd, IOCTL_USBFS_SUBMITURB, urb);
//    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "brightness hola %d", dpriv->fd);
